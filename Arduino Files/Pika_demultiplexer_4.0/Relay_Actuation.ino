  ...
  for (int j = 0; j < 5; j++) {                             // We run through all the values of the Binary label (5 values)
    ...
    for (int k = 0; k < 4; k++) {                           // We loop through all the pins stored within the array (maximum of 4)
      ...
      if (Set_Pin_To_Actuate != 0) {                        // We only actuate the pin if the pin number is not 0
        #ifdef Debugging
        Print_Pin_Status(Reset_Pin_To_Actuate, Pin_state);  // If we are debugging we print the state of each pin
        #endif
        digitalWrite(Set_Pin_To_Actuate, Pin_state);        // We write the state of the current Binary label number to the selected Set pin
      }
      ...
      if (Reset_Pin_To_Actuate != 0) {                      // We only actuate the pin if the pin number is not 0
        ...
        Print_Pin_Status(Reset_Pin_To_Actuate, Pin_state);      // If we are debugging we print the state of each pin
        #endif
        digitalWrite(Reset_Pin_To_Actuate, invert(Pin_state));  // We write the inverse state of the current Binary label number to the selected Reset pin
      }
    }
...
void Print_Pin_Status(int Pin_Number, int Pin_State)
{
